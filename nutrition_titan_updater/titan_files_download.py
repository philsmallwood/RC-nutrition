                ...
                if daily_nutrition_files:
                    for file in daily_nutrition_files:
                        if (file[-3:]=='xls'):
                            ...
                            log_entry += log_file_download(log_entry,"Urban Promise")
                            try:
                                sftp.remove(file)
                            except:
                                log_entry += log_script_error_cannot_remove(log_entry)
                        elif (file[-4:]=='xlsx'):
                            sftp.get(file, nutrition_urban_promise_path + file)
                            log_entry += log_file_download(log_entry,"Urban Promise")
                            try:
                            ...
                            #Logging
                            log_entry += "------------------\n"
                            log_entry += "File Skipped - Not an Urban Promise File \n"
                            log_entry += "------------------\n"
                else:
                    #Logging
                    log_entry += "------------------\n"
                ...
                if direct_cert_file:
                    for file in direct_cert_file:
                        if (file[-3:]=='txt'):
                            ...
                            log_entry += log_file_download(log_entry,"DirCert")
                            try:
                                sftp.remove(file)
                            except:
                                log_entry += log_script_error_cannot_remove(log_entry)
                            ...
                            #Logging
                            log_entry += "------------------\n"
                            log_entry += "File Skipped - Not a Direct Cert File \n"
                            log_entry += "------------------\n"
                else:
                    #Logging
                    log_entry += "------------------\n"
